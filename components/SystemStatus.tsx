...
export default function SystemStatus({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const runAllChecks = useCallback(async () => {
    ...
    {
      const out = await runCmd("echo 'APPIDE_OK' && uname -srm", session, TERMINAL_API);
      const ok = out.includes("APPIDE_OK");
      update("terminal", {
        stat: ok ? "ok" : "fail",
        detail: ok ? "Comandos Linux executam de verdade ✅" : "Terminal sem servidor — use Termux ou IDE na nuvem",
        realOutput: out,
      });
    }
